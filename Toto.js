...
function prizepool(pool_dict) {
  ...
  var minimum =
    probabilities[Math.floor(Math.random() * probabilities.length)] * 10000;

  // Gets respective prizepool's step i.e group1 = 100, group2 = 10
  var step;
  if (pool_dict == group1) {
    step = 100;
  } else if (pool_dict == group2) {
    step = 10;
  } else if (pool_dict == group3) {
    step = 5;
  } else if (pool_dict == group4) {
    step = 2;
  }
...
function sharedwinners(winners_dict) {
  ...
  var step;
  if (winners_dict == group1_winners) {
    step = 1;
  } else if (winners_dict == group2_winners) {
    step = 1;
  } else if (winners_dict == group3_winners) {
    step = 20;
  } else if (winners_dict == group4_winners) {
    step = 100;
  }
...
function generateNumber(betLength, fromHere) {
  if (betLength == "winningNumbers") {
    ...
    while (chosenNumbers.length != 6) {
      var chosen =
        functioningPool[Math.floor(Math.random() * functioningPool.length)];
...
function winningPercentage() {
  ...
  return [
    winningPercentagePerBet.toPrecision(5),
    winningPercentagePerDraw.toPrecision(5)
  ];
}
...
function checkUserInput(string, betLength) {
  ...
  if (betLength > 12 || betLength < 6) {
    ...
    pass = false;
    printOnScreen(
      "Make sure you keyed in ".concat(betLength).concat(" numbers per bet.")
    );
    ...
    string.forEach(element => {
      ...
      if (minirun == betLength) {
        ...
        if (betLength != 6) {
          generateNumber("yes", bet).forEach(element =>
            playerBet.push(element)
          );
        } else {
          playerBet.push(bet);
        }
...
function loop() {
  if (criteria == "Jackpot" || criteria > 399) {
    for (var miniruns = 0; miniruns < limit; miniruns++) {
      ...
      if (prizeGroup[0] == 1 || runs == criteria) {
        break;
      }
    }
  } else {
...
function updateResults(string) {
  ...
  context.fillText(runs, 250, canvas_height * 0.8);

  if (string == "final") {
    context.fillText("Number of Years: ", 250, canvas_height * 0.85);
    context.fillText(Math.round(runs / 104), 250, canvas_height * 0.9);
    ...
    if (criteria == "Jackpot") {
      context.fillText(
        "Chance of getting Jackpot: 1/".concat(runs),
        canvas_width / 2,
        canvas_height * 0.98
      );
    }
...
function printOnScreen(string) {
  ...
  context.font = "15px Arial";
  context.fillText(
    'Key in your bets below, separated by "[ ],"',
    canvas_width / 2,
    canvas_height / 4
  );
  context.fillText(
  ...
  context.fillText(
    "Key in the number of simulations you want to run.",
    canvas_width / 2,
    canvas_height / 2.6
  );
  context.fillText(
...
function generateBets(x) {
  let inputVal = [];
  for (let i = 0; i < x; i++) {
    inputVal.push(Math.floor(Math.random() * (50 - 1) + 1));
  }
  return inputVal;
}

function quickPick() {
  var sysBetVal = document.getElementById("betLength").value;
  var inputEl = document.getElementById("Bet");
  if (sysBetVal >= 7 && sysBetVal <= 12 && sysBetVal != "") {
    // quickpick with system bet
    inputEl.value = generateBets(sysBetVal);
  } else {
    console.log("default 6 numbers");
    // default 6 numbers QP
    inputEl.value = generateBets(6);
  }
}
